...
function BioStep() {
  ...
  return (
    <SignupShell
      ...
      subtitle="Write a short bio so people know who you are"
      back="/signup/gender"
      footer={<NeonButton disabled={bio.trim().length < 3} onClick={() => nav({ to: "/signup/done" })}>Finish</NeonButton>}
    >
      <textarea
        autoFocus
        value={bio}
        onChange={(e) => setBio(e.target.value.slice(0, max))}
        placeholder="I love exploring the metaverse..."
        rows={5}
